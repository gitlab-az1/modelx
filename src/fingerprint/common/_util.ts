/**
 * @copyright FingerprintJS
 * @see https://github.com/fingerprintjs
 */
...
export function isChromium122OrNewer(): boolean {
  ...
  const w = window as Window & typeof globalThis & { URLPattern?: new () => void };
  const { URLPattern } = w || { URLPattern: { } };
...
export function isAndroid(): boolean {
  ...
  if(isItGecko) return countTruthy(['onorientationchange' in w, 'orientation' in w, /android/i.test(n.appVersion)]) >= 2;
