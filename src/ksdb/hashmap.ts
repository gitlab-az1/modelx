...
function consumeStoreStream(stream: Readable, token: ICancellationToken = CancellationToken.None): Promise<PartialWrappedStore> {
  ...
  return new Promise((resolve, reject) => {
    token.onCancellationRequested(reason => {
      stream.destroy();
      [headerChunks, metadataChunks, payloadChunks].forEach(s => void s.end());
...
type StoreInitState = {
  prevContent: PrimitiveDictionary;
}
...
export class HashMapStore<V extends string | number | boolean | null | { valueOf(): string }, K extends string | symbol | { valueOf(): string }> {
  public static async fromPath<V extends string | number | boolean | null | { valueOf(): string }, K extends string | symbol | { valueOf(): string }>(storePath: string): Promise<HashMapStore<V, K>> {
    ...
    const { payload: prevContent } = await consumeStoreStream(fs.createReadStream(storePath));
    return new HashMapStore({ prevContent });
  }

  public static empty<V extends string | number | boolean | null | { valueOf(): string }, K extends string | symbol | { valueOf(): string }>(): HashMapStore<V, K> {
    return new HashMapStore({ prevContent: { } });
  }
  ...
  private constructor(_initState: StoreInitState) {
    ...
    for(const prop in _initState.prevContent) {
      this.set(prop as K, _initState.prevContent[prop] as V);
    }
  }
